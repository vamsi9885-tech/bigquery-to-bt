...
class HubBase(ABC):
    ...
    def __init__(
        ...
        hive_partitions: str = "source_system_cd,active_ym",
    ) -> None:
    ...
    def join_table(self, df_dict, driving_table, join_table, join_condition, join_type = "left"):
        return driving_table.join(
            df_dict[join_table].alias(join_table),
            on=join_condition,
            how=join_type
        )
    ...
    def run(self) -> None:
        ...
        target_db = self.args.get("target_db").replace("{db_env}", db_env)
        active_dt = self.args.get("start_date","1111-11-11")
