...
from typing import Dict, Union, List, Tuple, Mapping, Callable, Optional
...
# This function converts the string date to date format.
def str_to_date(str_dt, dt_frmt):
    return datetime.strptime(str_dt, dt_frmt)

# dt_format_convert('04AUG2020:00:09:13.123', '%d%b%Y:%H:%m:%S.%f', '%Y-%M-%d %H:%m:%S.%f')
def dt_format_convert(str_dt, from_format, to_format):
    if str_dt is not None and str_dt != '':
        dt = str_to_date(str_dt, from_format)
        return dt.strftime(to_format)
    else:
        return None


def load_tables(hc: SparkSession, table_columns: Mapping[str, Union[str, List[str], Dict[str, str]]], active_dt: str,
                db_name: Optional[str] = None, db_env: Optional[str] = None, table_loader: Callable = selective_df) -> \
Dict[str, DataFrame]:
    """Load tables based on the provided configuration.
    This function reads data from the specified tables and applies filtering based on the active date.
`
    Args:
        hc (SparkSession): Spark Session
        table_columns (Dict[str, Union[str, List, Dict]]): table configurations
        active_dt (str): active date to filter the data
        db_name (str, optional): Source database if not included in the table name in the configuration. Defaults to None.
        db_env (str, optional): The first letter of the DB (e.g., 'd','q','u','p').
        table_loader (function, optional): Function to load the table. Defaults to selective_df.

    Returns:
        Dict[str, DataFrame]: Dictionary of DataFrames.
    """
    ds = {}
    for tbl_key, col_cfg in table_columns.items():
        tbl_name, df_name = _parse_db_tbl_and_df_name(tbl_key, col_cfg, db_name, db_env)
        ds[df_name] = table_loader(hc, tbl_name, col_cfg, active_dt)

    return ds


def _parse_db_tbl_and_df_name(tbl_key: str,
                              col_cfg: Union[str, List[str], Dict[str, str]],
                              db_name: Optional[str] = None,
                              db_env: Optional[str] = None) -> Tuple[str, str]:
    """Parse the database table name and DataFrame name from the given table key.

    Args:
        tbl_key (str): The table key, which may contain the database name and table name.
        db_name (str, optional): The database name to use if not included in the table key. Defaults to None.
        db_env (str, optional), optional): The first letter of the DB (e.g., 'd','q','u','p'). Defaults to None.

    Returns:
        Tuple[str,str]: A tuple containing the database table name and DataFrame name.
    """
    if db_env:
        tbl_key = tbl_key.replace("{db_env}", db_env)

    # Get database name
    if "." in tbl_key:
        # if db_table contains a dot, then the first part is used as the database name
        db_name = tbl_key.split(".")[0]
    elif db_name is None:
        # if tbl_key doesn't contain database name and db_name is not provided, then raises an exception
        raise Exception("db_name argument is not provided")

    # Get df_name
    df_name = (tbl_key.split(".")[1] if "." in tbl_key else tbl_key.lower())
    # Get table name
    tbl_name = tbl_key.split(".")[1] if "." in tbl_key else tbl_key.lower()

    if isinstance(col_cfg, Dict) and "source_table" in col_cfg:
        # if source_table is provided in the column configuration, then it is used as the table name
        tbl_name = col_cfg["source_table"]

    db_table = db_name + '.' + tbl_name

    return db_table, df_name
