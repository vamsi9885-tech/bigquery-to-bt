...
class FAM:
    ...
    def get_records_without_cadence_id(self):
        return self.utils_obj.run_select_query(f""" SELECT distinct feed_name, window_start_date, extraction_type 
            FROM file_arrival_status where  expectation_date < '{date.today()}' 
            and (('{date.today()}' BETWEEN yellow_email_start_date AND yellow_email_end_date) 
                OR ('{date.today()}' BETWEEN red_email_start_date AND red_email_end_date)) 
            and cadence_id is null group by feed_name, window_start_date, extraction_type""")

    def get_client_names(self):
        return self.utils_obj.run_select_query(f"""SELECT distinct client_name FROM file_arrival_status f inner join file_master d on f.cadence_id = d.cadence_id and f.file_name_format = d.file_name_format  where d.arrived_flag is not true and f.expectation_date < '{date.today()}' and (('{date.today()}' BETWEEN f.yellow_email_start_date AND f.yellow_email_end_date) OR ('{date.today()}' BETWEEN f.red_email_start_date AND f.red_email_end_date))""")
    ...
    def get_file_arrival_status(self, clientname):
        return self.utils_obj.run_select_query(f"""SELECT f.cadence_id, f.client_name, f.feed_id, f.feed_name, f.file_name_format, f.part, f.is_mandatory
            ...
            where f.client_name = '{clientname}' and d.arrived_flag is not true and f.expectation_date < '{date.today()}' 
            and (('{date.today()}' BETWEEN f.yellow_email_start_date AND f.yellow_email_end_date) 
                OR ('{date.today()}' BETWEEN f.red_email_start_date AND f.red_email_end_date)) order by f.expectation_date, f.file_name_format, f.part""")
    ...
    def calculate_status_for_files(self, data_with_cadence):
        ...
        for row in data_with_cadence:
            ...
            logging.debug("current_date", extra={"properties": {"current_date": str(date.today())}})
            if(yellow_email_start_date is not None and yellow_email_end_date is not None and (yellow_email_start_date <= date.today() <= yellow_email_end_date)):
                if is_mandatory:
                    updated_row = row[:11] + ("Late",) + row[12:]
                else:
                    updated_row = row[:11] + ("Warning",) + row[12:]
                updated_status = updated_row
            elif(red_email_start_date is not None and red_email_end_date is not None and (red_email_start_date <= date.today() <= red_email_end_date)):
                if is_mandatory:
    ...
    def update_status_and_sentdate(self, cadence_id, status, file_name_format, part):
        try:
            query = f"""UPDATE file_master set status = '{status}', notification_sent_date = '{date.today()}' 
