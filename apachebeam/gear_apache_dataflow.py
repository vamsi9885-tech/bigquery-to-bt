...
with DAG(dag_id="gear_bq_to_bt_java", #DAG Name 
    ...
    GCS_deletion = DataprocSubmitJobOperator(task_id="gcs_deletion", pyspark_job=POST_PYSPARK_JOB, retries=0)
    # Delete Cluster
    delete_cluster = DataprocDeleteClusterOperator(task_id="delete_cluster", cluster_name=cluster_name)

    # Order of Execution
    create_cluster >> bigtable_GCS >> submit_dataflow_job >> GCS_deletion >> delete_cluster
    # create_cluster  >> GCS_deletion >> delete_cluster